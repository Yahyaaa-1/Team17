...
class RakusensAuthAPI:
    def __init__(self, app):
        self.app = app
        self.setup_routes()

    def setup_routes(self):
        @self.app.route('/api/verify-employee', methods=['POST', 'OPTIONS'])
...
class RakusensSensorAPI:
    def __init__(self, app):
        self.app = app
        self.setup_routes()

    def setup_routes(self):
        @self.app.route('/api/live-data/<line>', methods=['GET'])
        def get_live_data(self, line)
            try:
                ...
                if connection:
                    connection.close()
        @self.app.route('/api/average-temperature/<line>', methods=['POST'])
        def get_average_temperature(self, line)
            try:
                connection = get_db_connection()
                if not connection:
                    return jsonify({'error': 'Database connection failed'}), 500
    ...
    def setup_routes(self):
        ...
        def trigger_log(self, )
            ...
            try:
                ...
                return jsonify({'success': False, 'error': str(e)}), 500
        def log_event(self, message, type, log_level)
            try:
...
class RakusensAdminAPI:
    def __init__(self, app):
        self.app = app
        self.setup_routes()

    def setup_routes(self):
        @self.app.route('/api/admin/user-accounts', methods=['GET', 'OPTIONS'])
        ...
        # Get all employees
        @self.app.route('/api/admin/employee-reg', methods=['GET', 'OPTIONS'])
        def get_employee_reg(self, )
        ...
        # Delete employee
        @self.app.route('/api/admin/delete-employee', methods=['POST', 'OPTIONS'])
...
def create_app():
    ...
    RakusensAuthAPI(app)
    RakusensUserSettingsAPI(app)
    RakusensSensorAPI(app)
    RakusensLogsAPI(app)
    RakusensAdminAPI(app)
    return app
