...
const SENSOR_CONFIG = {
    line4: ['r01', 'r02', 'r03', 'r04', 'r05', 'r06', 'r07', 'r08'],
    line5: ['r01', 'r02', 'r03', 'r04', 'r05', 'r06', 'r07', 'r08', 
            'r09', 'r10', 'r11', 'r12', 'r13', 'r14', 'r15', 'r16', 'r17']
};

// Helper Functions
async function getActiveSensors(line) {
    try {
        const response = await fetch(`http://127.0.0.1:5000/api/admin/table-headers?tableID=${line}`);
        const data = await response.json();
        if (data.success) {
            // Only return sensors that exist in both config and database
            return SENSOR_CONFIG[line].filter(sensor => 
                data.headers.includes(sensor))
                .sort();
        }
        return SENSOR_CONFIG[line]; // Fallback to full config if API fails
    } catch (error) {
        console.error('Error fetching sensors:', error);
        return SENSOR_CONFIG[line]; // Fallback to full config
    }
}
...
async function initializeDonutSlider(line) {
    ...
    const activeSensors = await getActiveSensors(line);
    ...
    maxSlides = Math.max(0, activeSensors.length - 1);
}
...
async function updateSensorSelector(line) {
    ...
    if (activeSensors.includes(currentSelectedValue)) {
        ...
        selector.value = activeSensors[0];
    }
...
function updateDonutCharts(liveData) {
    // First remove any donut charts for sensors that no longer exist in liveData
    Object.keys(donutCharts).forEach(sensor => {
        if (!(sensor in liveData)) {
            // Remove the chart container from DOM
            const chartElement = document.getElementById(`donut-chart-${sensor}`);
            if (chartElement && chartElement.parentNode) {
                chartElement.parentNode.remove();
            }
            // Remove from donutCharts object
            delete donutCharts[sensor];
        }
    });
    ...
    Object.keys(donutCharts).forEach(sensor => {
        if (liveData[sensor] !== undefined) {
            ...
            donutCharts[sensor].updateOptions({ colors: ['#CCCCCC'] });
            donutCharts[sensor].updateSeries([0]);
        }
    });
}

function updateBarChart(liveData) {
    const sensors = Object.keys(donutCharts); // Only use sensors that have donut charts
    const categories = sensors.map(s => s.toUpperCase());
    const seriesData = sensors.map(s => liveData[s] || 0);
...
function updateAnalytics(liveData) {
    const sensors = Object.keys(donutCharts); // Only count sensors that have donut charts
    ...
    const averageTemperature = totalSensors > 0 ? (totalTemperature / totalSensors).toFixed(2) : 0;
